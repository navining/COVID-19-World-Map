...
function drawCircles2() {
  ...
  for (var id in countries) {
    ...
    if (country.recovered == 0) continue;
    var lat, lon;
    circles
      .append("circle")
      .attr("id", country.id)
      ...
      .attr("r", function () {
        return 3 + country.recovered / 400000;
      })
      .attr("fill", "red")
      .attr("stroke", "black")
      .attr("opacity", 0.5)
    ...
    circles
      ...
      .text(country.name + ": " + country.recovered)
      .attr("x", lon + 20)
      .attr("y", lat - 20)
...
function visualize2() {
  $.getJSON("data/world_countries.json", drawMap2);
  drawText2();
  drawCircles2();
  setUpZoom2();
}
